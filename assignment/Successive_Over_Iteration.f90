subroutine SOR(aIn, n, xGuess, tolerance, x)
    implicit none
    ...
    do while(solutionFound .eqv. .false.)
         do rowCount = 1, n
            factor = 0
            do columnCount = 1, n
                if(columnCount /= rowCount .AND. columnCount > rowCount) then
                    factor = factor + a(rowCount, columnCount) *xPrevious(columnCount)
                else if(columnCount /= rowCount .AND. columnCount < rowCount) then
                    factor = factor + a(rowCount, columnCount) *x(columnCount)
                end if
            end do
            x(rowCount) = (1 / a(rowCount, rowCount)) * (a(rowCount, (n+1)) - factor)
            !xPrevious(rowCount)=x(rowCount)
        end do
        do rowCount =1,n
            x(rowCount)=xPrevious(rowCount)+1.5*(x(rowCount)-xPrevious(rowCount))
        end do 
        do rowCount = 1, n
...
a(5,6) = -201.9
xGuess = 0
call SOR(a, n, xGuess, tolerance, x)
write(*,*) "Solution by Successive Over Iteration Method:"
call printMatrix2D(x, n, 1)
